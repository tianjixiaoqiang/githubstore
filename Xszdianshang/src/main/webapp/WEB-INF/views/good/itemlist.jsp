<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" import="entity.Good"%>
...
<body>
	...
	<div class="container">
		...
		<!--banner end -->
		<div class="list_dq">
			<%-- <a href="" target="_blank">慧慧购书网</a> > <c:if test="${good.grade == 1}"><a href="good?method=findBygoodField&currentPage=1&specialty=1" target="_self">小学</a> >小学一年级</c:if> --%>
			<a href="Good/toIndex" target="_blank">首页</a> > 图书 
		</div>
		<!--fenlei begin -->
		<div class="list_fl">
		...
		<c:if test="${not empty good.specialty}">
			<c:set var="condition" value="specialty=${good.specialty}"></c:set>
		</c:if>
		<c:if test="${not empty good.grade}">
			<c:set var="condition" value="grade=${good.grade}"></c:set>
		</c:if>
		<c:if test="${not empty good.sold}">
			<c:set var="condition2" value="sold=${good.sold}"></c:set>
		</c:if>
		<c:if test="${not empty good.price}">
			<c:set var="condition2" value="price=${good.price}"></c:set>
		</c:if>
		<c:if test="${not empty good.pubdate}">
			<c:set var="condition2" value="pubdate=${good.pubdate}"></c:set>
		</c:if>
		<c:if test="${not empty good.goodsname}">
			<c:set var="condition2" value="${condition2}&name=${good.goodsname}"></c:set>
		</c:if>
		<!--paixu begin -->
		<div class="paixu">
			<div class="number">
			...
			<ul>
				<li class="li1">排序</li>
				<li id="a_default"><a href="Good/sortDefault">默认排序</a></li>
				<li id="a_sold"><a
					href="Good/sortBySold">销量<b><img
							src="images/list_jtx.jpg" width="7" height="8" /></b></a></li>
				<li id="a_price"><a
					href="Good/sortByPrice">价格<b><img
							src="images/list_jts.jpg" width="7" height="8" /></b></a></li>
				<li id="a_pubdate"><a
					href="Good/sortByDate">出版时间<b><img
							src="images/list_jtx.jpg" width="7" height="8" /></b></a></li>
			</ul>
			<!-- 排序的选中状态 -->
		...
		<div class="today">
			...
			<div class="today_b">
				<!-- 商品列表开始 -->
				<ul>
					<c:forEach items="${page.rows.subList(page.beginIndex,page.endIndex)}" var="good">
						<li><a href="">
								<div class="pics">
									<img src="${good.imgurl }" width="178" height="168" alt=""
										title="" />
								</div>
								<div class="tt">${good.goodsname }</div>
								<style>
											.tt{
											overflow: hidden;
											white-space: nowrap;
								...
								<div class="no_price">
									<span class="dj">定价：<del>
											<fmt:formatNumber value="${good.price}" type="currency" />
										</del></span><span class="yhj">优惠价<b><fmt:formatNumber
												value="${good.price * good.discount }" type="currency" /></b></span>
								</div>
						</a></li>
					</c:forEach>
				</ul>
		...
		<div class="pages">
			第${page.currentPage}页/共${page.totalPages}页&nbsp;&nbsp;&nbsp;&nbsp; <a
				href="Good/list?&currentPage=1&${condition}&${condition2}">首页</a>&nbsp;&nbsp;
			<c:if test="${page.currentPage>1}">
				<a
					href="Good/list?${condition}&currentPage=${page.currentPage-1}&${condition2}">上一页</a>&nbsp;&nbsp;
		 </c:if>
			<%-- 中间的页数 --%>
			<c:choose>
			...
			<c:forEach var="index" begin="${beginIndex}" end="${endIndex}">
				<c:choose>
					...
					<c:otherwise>
						<a
							href="Good/list?${condition}&currentPage=${index}&${condition2}">${index}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:if test="${page.currentPage<page.totalPages}">
				<a
					href="Good/list?${condition}&currentPage=${page.currentPage+1}&${condition2}">下一页</a>&nbsp;&nbsp;
		 </c:if>
			<a
				href="Good/list?${condition}&currentPage=${page.totalPages}&${condition2}">尾页</a>&nbsp;&nbsp;
			/&nbsp;&nbsp;共${page.total}条记录
